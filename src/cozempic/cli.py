...
def cmd_digest(args):
    from .digest import (
        ...
        load_digest_store, recover_digest, remove_synced_memory,
    ...
    # exists to tell the CLI user why nothing happened, and to clean up a
    # previously synced memory on the hook-driven inject/recover calls.
    if action in ("update", "flush", "inject", "recover") and not digest_enabled():
        cwd = getattr(args, "cwd", None) or os.getcwd()
        if remove_synced_memory(cwd):
            print(
                "Behavioral digest disabled (COZEMPIC_NO_DIGEST); "
                "removed previously synced memory."
            )
        else:
